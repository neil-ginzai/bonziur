...
`,
    ...
    function setup(logindata) {
        ...
        socket.on("dvdbounce", (data) => {
            let ag = agents[data.guid];
            if (!ag) return;
            if (ag.dvdbounceInterval) {
                clearInterval(ag.dvdbounceInterval);
                delete ag.dvdbounceInterval;
                return;
            }
            let vx = (Math.random() > 0.5 ? 1 : -1) * (2 + Math.random() * 2);
            let vy = (Math.random() > 0.5 ? 1 : -1) * (2 + Math.random() * 2);
            ag.dvdbounceInterval = setInterval(() => {
                ag.x += vx;
                ag.y += vy;
                if (ag.x <= minx) { ag.x = minx; vx = Math.abs(vx); }
                if (ag.x >= innerWidth - ag.w) { ag.x = innerWidth - ag.w; vx = -Math.abs(vx); }
                if (ag.y <= 0) { ag.y = 0; vy = Math.abs(vy); }
                if (ag.y >= innerHeight - 32 - ag.h) { ag.y = innerHeight - 32 - ag.h; vy = -Math.abs(vy); }
                ag.update();
            }, 16);
        });
        socket.on("fly", (data) => {
    ...
    function getFingerprint() {
        try {
            ...
            ctx.font = "14px Arial";
            ctx.fillText("BonziWorld fp", 2, 2);
            const canvasFP = canvas.toDataURL().slice(-40);
            const raw = [
                navigator.userAgent,
                navigator.language,
                screen.colorDepth,
