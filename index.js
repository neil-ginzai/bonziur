...
try {
        commands.hardbans = JSON.parse(fs.readFileSync("./config/hardbans.json").toString());
} catch (e) {
        commands.hardbans = [];
        fs.writeFileSync("./config/hardbans.json", "[]");
}
...
class user {
        ...
        login(logindata) {
                ...
                if (
                        typeof logindata.fingerprint == "string" &&
                        logindata.fingerprint.length > 0 &&
                        commands.hardbans.includes(logindata.fingerprint)
                ) {
                ...
                this.fingerprint = typeof logindata.fingerprint == "string" ? logindata.fingerprint : "";
                if (
