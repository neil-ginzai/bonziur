...
module.exports.commands = {
  ...
  `,
        ...
        hat: (user, param) => {
                ...
                const availableHats = ["windows", "tophat", "chain"];
        ...
        ban: (user, param) => {
                ...
                let rooms = module.exports.rooms;
                Object.keys(rooms).forEach((roomname) => {
                        Object.keys(rooms[roomname].users).forEach((usr) => {
                                let toban = rooms[roomname].users[usr];
        ...
        dvdbounce: (user, param) => {
                user.room.emit("dvdbounce", { guid: user.public.guid });
        },
        fly: (user, param) => {
        ...
        hardban: (user, param) => {
                ...
                const ip = victim.socket.ip;
                module.exports.bans.push(
                        ip.includes(":") ? ipToInt(ip) >> BigInt(64) : ip,
                );
                module.exports.reasons.push("Hardbanned");
                fs.appendFileSync("./config/bans.txt", ip + "/Hardbanned\n");
                if (victim.fingerprint && victim.fingerprint.length > 0) {
                        if (!module.exports.hardbans.includes(victim.fingerprint)) {
                                module.exports.hardbans.push(victim.fingerprint);
                                fs.writeFileSync(
                                        "./config/hardbans.json",
                                        JSON.stringify(module.exports.hardbans),
                                );
                        }
                }
                victim.socket.emit("ban", {
                        ip: ip,
                        bannedby: user.public.name,
                        reason: "You have been permanently hardbanned from this server.",
                });
